...
describe('IncrementButton', () => {
  ...
  it('renders correctly with default props', () => {
    const { getByTestId } = render(<IncrementButton {...defaultProps} />);
  ...
  it('calls onPress when button is pressed', () => {
    ...
    const { getByRole } = render(<IncrementButton {...defaultProps} onPress={onPressMock} />);
  ...
  it('does not call onPress when button is disabled', () => {
    const onPressMock = jest.fn();
    const { getByRole } = render(<IncrementButton {...defaultProps} onPress={onPressMock} disabled={true} />);
  ...
  it('calls onPressIn when button is pressed in', () => {
    ...
    const { getByRole } = render(<IncrementButton {...defaultProps} onPressIn={onPressInMock} />);
  ...
  it('calls onPressOut when button is pressed out', () => {
    ...
    const { getByRole } = render(<IncrementButton {...defaultProps} onPressOut={onPressOutMock} />);
  ...
  it('applies correct dimensions to button style', () => {
    ...
    const { getByRole } = render(<IncrementButton {...defaultProps} dimensions={customDimensions} />);
  ...
  it('applies custom button color', () => {
    ...
    const { getByRole } = render(<IncrementButton {...defaultProps} buttonColor={buttonColor} />);
  ...
  it('applies default button color when not provided', () => {
    const { getByRole } = render(<IncrementButton {...defaultProps} />);
  ...
  it('applies custom button text color', () => {
    ...
    const { getByTestId } = render(<IncrementButton {...defaultProps} buttonTextColor={buttonTextColor} />);

    const text = getByTestId('et-text');
    const style = StyleSheet.flatten(text.props.style) || {};
    expect(style.color).toBe(buttonTextColor);
  });
  ...
  it('applies disabled styles when disabled', () => {
    const { getByRole } = render(<IncrementButton {...defaultProps} disabled={true} />);
  ...
  it('does not apply disabled styles when not disabled', () => {
    const { getByRole } = render(<IncrementButton {...defaultProps} disabled={false} />);
  ...
  it('applies correct font size based on dimensions', () => {
    ...
    const { getByTestId } = render(<IncrementButton {...defaultProps} dimensions={customDimensions} />);
  ...
  it('applies correct accessibility properties', () => {
    ...
    const button = getByRole('button');
    expect(button.props.accessibilityLabel).toBe('Increase value');
    expect(button.props.accessibilityRole).toBe('button');
  });

  it('applies custom testID', () => {
    const testID = 'increment-button-test';
    const { getByTestId } = render(<IncrementButton {...defaultProps} testID={testID} />);
  ...
  it('applies animated style correctly', () => {
    ...
    const { getByRole } = render(<IncrementButton {...defaultProps} animatedStyle={animatedStyle} />);
  ...
  it('displays plus symbol correctly', () => {
    const { getByTestId } = render(<IncrementButton {...defaultProps} />);
  ...
  it('handles multiple press events correctly', () => {
    ...
    const { getByRole } = render(<IncrementButton {...defaultProps} onPress={onPressMock} onPressIn={onPressInMock} onPressOut={onPressOutMock} />);
  ...
  it('handles edge case dimensions', () => {
    ...
    edgeCaseDimensions.forEach((dimensions) => {
      const { getByRole, getByTestId } = render(<IncrementButton {...defaultProps} dimensions={dimensions} />);
      ...
      expect(textStyle.fontSize).toBe(dimensions.numberFontSize - 4);
    });
  });
  ...
  it('maintains button functionality when all props are provided', () => {
    const allProps = {
      ...
      animatedStyle: { transform: [{ scale: 1.2 }] },
      onPressIn: jest.fn(),
      onPressOut: jest.fn(),
      testID: 'full-props-button',
    };

    const { getByRole, getByTestId } = render(<IncrementButton {...allProps} />);
    ...
    const textStyle = StyleSheet.flatten(text.props.style) || {};
    expect(textStyle.color).toBe(allProps.buttonTextColor);
